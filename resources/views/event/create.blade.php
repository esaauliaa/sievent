...
function muatRuanganTersedia() {
    ...
    const routeUrl = '{{ route("event.getRuanganByTanggal") }}';
    const url = new URL(routeUrl, window.location.origin);
    const currentParams = new URLSearchParams(window.location.search);

    if (currentParams.has('_tab')) {
        url.searchParams.set('_tab', currentParams.get('_tab'));
    }
    ...
    fetch(url.toString(), { credentials: 'same-origin' })
        ...
        .catch(error => {
            console.error('Fetch error saat memuat ruangan:', error);
