...
@section('title', 'Kehadiran Saya - SiEvent UNEJ')
    ...
    <div class="max-w-7xl mx-auto space-y-6">
        <div class="bg-gradient-to-r from-[#0056B3] to-[#131D4F] rounded-3xl p-8 text-white shadow-lg relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <h2 class="text-3xl font-extrabold mb-3">Kehadiran Saya</h2>
                    <p class="text-white/85 max-w-2xl leading-relaxed">
                        Lihat status presensi untuk event yang sudah kamu ikuti.
                    </p>
                </div>
                ...
                <a href="{{ route('event.index') }}"
                   class="px-6 py-3 rounded-2xl bg-white text-[#0056B3] hover:bg-[#e1effe] transition font-bold shadow-md inline-flex items-center justify-center">
                    <i class="fa-solid fa-calendar-plus mr-2"></i> Cari Event
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-white border border-blue-100 p-5 rounded-3xl shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-bold text-gray-400 uppercase">Event Diikuti</p>
                    <p class="text-2xl font-black text-blue-900">{{ $totalEventDiikuti }}</p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-3xl border border-blue-100 shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-blue-100">
                <h3 class="text-lg font-extrabold text-[#131D4F]">Rekap Event Saya</h3>
                <p class="text-sm text-gray-500 mt-1">Status akan berubah setelah admin menandai presensimu.</p>
            </div>
                ...
                <table class="w-full text-left">
                    <thead class="bg-[#e1effe] text-[#131D4F]">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-sm font-extrabold">Tanggal Event</th>
                    ...
                    <tbody class="divide-y divide-blue-50 text-sm text-gray-700">
                        @forelse($presensis as $index => $presensi)
                            @php
                                ...
                                $isHadir = $presensi->status_kehadiran === 'hadir';
                            @endphp
                            <tr class="hover:bg-slate-50/50 transition">
                                <td class="px-6 py-4 text-center text-gray-400 font-bold">{{ $index + 1 }}</td>
                                <td class="px-6 py-4">
                                    <p class="font-extrabold text-[#131D4F]">{{ $event->nama_event ?? 'Event tidak ditemukan' }}</p>
                                    <p class="text-xs text-gray-400 mt-0.5">Daftar: {{ $presensi->tanggal_daftar ? $presensi->tanggal_daftar->translatedFormat('d M Y') : '-' }}</p>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center text-xs text-gray-500 font-mono">
                                    {{ $presensi->waktu_presensi ? $presensi->waktu_presensi->format('H:i:s') . ' WIB' : '-' }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($event)
                                        <a href="{{ route('event.show', $event->id_event) }}"
                                           class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-gray-100 text-[#131D4F] text-xs font-extrabold hover:bg-gray-200 transition">
